...
use crate::boundary::compute_bertero_weights;
use crate::config::{DwOpts, Metric};
use crate::iterator::DwIterator;
...
impl DeconvMethod for RlMethod {
    fn deconvolve<B: FftBackend>(
        ...
        // FFT of PSF (computed once, reused all iterations)
        let psf_fft = fft
            .forward(psf_expanded.as_slice())
            .map_err(|e| dw_core::DwError::Config(e.to_string()))?;

        // Compute Bertero boundary weights if border_quality > 0
...
fn initial_guess(
    ...
    match opts.start_condition {
        crate::config::StartCondition::Identity => {
            image.expand(wm, wn, wp)
        }
        crate::config::StartCondition::Flat | crate::config::StartCondition::LowPass => {
...
fn rl_iteration<B: FftBackend>(
    ...
    wn: usize,
    wp: usize,
) -> Result<f64, dw_core::DwError> {
    // Forward FFT of current guess
    let x_fft = fft
        .forward(x.as_slice())
        .map_err(|e| dw_core::DwError::Config(e.to_string()))?;

    // y = H * x (forward convolution)
    let y_data = fft
        .convolve(psf_fft, &x_fft)
        .map_err(|e| dw_core::DwError::Config(e.to_string()))?;
    let mut y = FimImage::from_vec(wm, wn, wp, y_data)?;

    // Compute error metric on the original region
    ...
    // Backward convolution: conj(PSF) * ratio
    let correction_data = fft
        .convolve_conj(psf_fft, &ratio_fft)
        .map_err(|e| dw_core::DwError::Config(e.to_string()))?;

    // Update: x = x * correction * W
            ...
            for pp in 0..p {
                for nn in 0..n {
                    for mm in 0..m {
                        let idx_im = pp * n * m + nn * m + mm;
                        let idx_y = pp * wn * wm + nn * wm + mm;
                        let d = im[idx_im] as f64 - y_slice[idx_y] as f64;
                        sum += d * d;
                        count += 1;
                    }
                }
            ...
            for pp in 0..p {
                for nn in 0..n {
                    for mm in 0..m {
                        let idx_im = pp * n * m + nn * m + mm;
                        let idx_y = pp * wn * wm + nn * wm + mm;
                        let a = im[idx_im] as f64;
                        let b = y_slice[idx_y] as f64;
                        if a > 0.0 && b > 0.0 {
                            sum += a * (a / b).ln() - a + b;
                        } else {
                            sum += b;
