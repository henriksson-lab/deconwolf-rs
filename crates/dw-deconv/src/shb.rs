...
use crate::config::{DwOpts, Metric, StartCondition};
...
/// Default method in deconwolf (Wang & Miller).
...
impl DeconvMethod for ShbMethod {
    fn deconvolve<B: FftBackend>(
        ...
        // Expand and shift PSF to job size
...
fn shb_iteration<B: FftBackend>(
    ...
    // Momentum: alpha = (k-1)/(k+2), clamped to [0, alpha_max]
    let alpha = if k > 1 {
    ...
    // Save current x as x_prev, then update x
